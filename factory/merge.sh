...
# The factory merges into DEVELOP and never main (program decision D2, FACTORY_RULES.md
# 2.6): develop -> main stays a human promotion, forever. Refusing the CONFIGURED value
# here means no combination of environment and defaults can point this script at main.
BASE_BRANCH="${FACTORY_BASE_BRANCH:-develop}"
if [ "$BASE_BRANCH" = "main" ]; then
  echo "MERGE_REFUSED: FACTORY_BASE_BRANCH is 'main'. The factory never merges to main"
  echo "  (FACTORY_RULES.md 2.6, program decision D2). develop -> main is Rick's call."
  exit 1
fi
...
if [ "$GH" -eq 1 ]; then
  git fetch --quiet origin "$BASE_BRANCH" "$BRANCH" || { echo "MERGE_REFUSED: cannot fetch from origin"; exit 1; }
  BASE="origin/$BASE_BRANCH"
  HEAD_REF="origin/$BRANCH"
else
  # File backend: the configured base when it exists, else main — which is what the
  # machinery fixture uses. `|| true` is load-bearing: an unguarded probe under `set -e`
  # would kill the script here with no output at all.
...
if [ "$GH" -eq 1 ]; then
  ...
  [ "$PR_BASE" = "$BASE_BRANCH" ] || { echo "MERGE_REFUSED: PR #$PR_NUM targets '$PR_BASE', not $BASE_BRANCH"; exit 1; }
  ...
  case "$PR_MERGE_STATE" in
    BLOCKED)
      ...
      echo "  Branch protection on '$BASE_BRANCH' is refusing this merge - typically a required"
      ...
      exit 1 ;;
    BEHIND)
      echo "MERGE_REFUSED: PR #$PR_NUM is BEHIND $BASE_BRANCH - protection requires branches to be"
      ...
      exit 1 ;;
    DIRTY)
      echo "MERGE_REFUSED: PR #$PR_NUM is DIRTY - GitHub sees a conflict with $BASE_BRANCH."
      exit 1 ;;
    UNKNOWN|"")
  ...
  TITLE="$(read_fm title)"
  # The BMAD trailers ride on the commit that actually lands on develop: the squash. The
  # review node emits them in the PR body; a PR without them gets a deterministic
  # fallback rather than a dead lap (FACTORY_RULES.md 2).
  TRAILER_BLOCK="$(python3 factory/state.py body "$PR_FILE" 2>/dev/null | grep -E '^(Epic|Story|Type|Learning|Affected):' || true)"
  [ -n "$TRAILER_BLOCK" ] || TRAILER_BLOCK="Epic: factory
Story: $ISSUE
Type: factory
Learning: none recorded
Affected: see PR #$PR_NUM"
  gh pr merge "$PR_NUM" --squash --delete-branch \
    --subject "${TITLE:-factory: $ISSUE}" \
    --body "Merged by factory/merge.sh from $BRANCH.
Issue: $ISSUE
Gates: contract, ratchet, e2e, guard -- all green on re-check.
...
$TRAILER_BLOCK" \
  ...
  # base that is one merge behind, and the guard's merge base is a commit the remote has
  # already moved past. Nothing errors; the diff just quietly contains someone else's work.
  git fetch --quiet origin "$BASE_BRANCH"
  # The remote factory/issue-N branch was just deleted, but its origin/* tracking ref
  # survives the targeted fetch above — all three Phase D laps left one behind (lap 3,
  # finding 2). Prune explicitly; a fetch with a refspec never prunes outside it.
  git remote prune origin >/dev/null 2>&1 || true
  if [ "$(git rev-parse --abbrev-ref HEAD)" = "$BASE_BRANCH" ]; then
    git merge --ff-only "origin/$BASE_BRANCH" >/dev/null 2>&1 || true
  else
    git update-ref "refs/heads/$BASE_BRANCH" "origin/$BASE_BRANCH" || true
  fi
  MERGED_SHA="$(git rev-parse --short "origin/$BASE_BRANCH")"
  ...
  LANDED="$(git log -1 --pretty=%s "origin/$BASE_BRANCH")"
  case "$LANDED" in
