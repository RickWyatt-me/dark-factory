...
while [ "$SLOTS" -gt 0 ]; do
  ...
  merge)
    ...
    if [ "$DRY_RUN" -eq 0 ]; then
      if bash factory/merge.sh "$TARGET"; then
        if bash factory/deploy.sh; then
          # Rick's directive 2026-08-17: a lap that finishes clean says so. The only
          # OTHER notification this factory sends is needs-human, so from a phone
          # "quiet" and "done" were indistinguishable. Deploy-failure semantics are
          # unchanged below: the merge stands, deploy.sh already escalated.
          log "$(factory_notify "$TARGET" "LAP DONE: merged and deployed, health checks green (develop @ $(git rev-parse --short origin/develop 2>/dev/null || echo new-head)). No action needed.")"
        else
          log "DEPLOY_FAILED after merging $TARGET - the merge stands; see the log above"
        fi
        # Phase G: the post-lap steps ride the same tail. Both are idempotent
        # catch-ups (a no-op is milliseconds), both fail soft — a missed review or
        # seed retries on any later tick and blocks nothing.
        bash factory/evolve.sh --catch-up || log "EVOLVE_FAILED - proposals wait; a later catch-up retries"
        bash factory/evolve-apply.sh --catch-up || log "EVOLVE_APPLY_FAILED - proposals stay proposed; a later catch-up retries"
        bash factory/brain-seed.sh --catch-up || log "SEED_FAILED - a later catch-up retries"
      else
  ...
  triage)
    if [ "$AUTONOMY" -lt 4 ]; then
      ...
      # A held tick dispatched nothing: it IS idle as far as the catch-up deploy is
      # concerned. Without this, one untriaged issue blocks the catch-up forever —
      # `next` answers with ONE thing, this hold consumes it, and the `idle` branch
      # below is never reached (notch 2->3: 11 overnight ticks held here behind
      # #18/#20 while a rolled-back deploy waited). Phase G seam 1, Rick-authorized.
      if [ "$DRY_RUN" -eq 0 ] && [ "$AUTONOMY" -ge 3 ]; then
        bash factory/deploy.sh || log "DEPLOY_FAILED during held tick - see the log above"
        # The TF road rides only the idle/held tails, never post-merge: N merged
        # laps since the last quiet moment coalesce into ONE build (Rick, 2026-08-18).
        bash factory/tf-build.sh || log "TF_BUILD_FAILED during held tick - pointer unmoved; next quiet tick retries"
        bash factory/evolve.sh --catch-up || log "EVOLVE_FAILED - proposals wait; a later catch-up retries"
        bash factory/evolve-apply.sh --catch-up || log "EVOLVE_APPLY_FAILED - proposals stay proposed; a later catch-up retries"
        bash factory/brain-seed.sh --catch-up || log "SEED_FAILED - a later catch-up retries"
      fi
  ...
  idle)
    ...
    # after a merge never notices that the last one silently stopped working. Phase G
    # added the post-lap catch-ups to the same slot, same reasoning: a review or seed
    # that only runs after a merge never covers the lap whose tick died first.
    log "nothing to do"
    if [ "$DRY_RUN" -eq 0 ] && [ "$AUTONOMY" -ge 3 ]; then
      bash factory/deploy.sh || true
      # The TF road rides only the idle/held tails, never post-merge: N merged
      # laps since the last quiet moment coalesce into ONE build (Rick, 2026-08-18).
      bash factory/tf-build.sh || log "TF_BUILD_FAILED during idle tick - pointer unmoved; next quiet tick retries"
      bash factory/evolve.sh --catch-up || log "EVOLVE_FAILED - proposals wait; a later catch-up retries"
      bash factory/evolve-apply.sh --catch-up || log "EVOLVE_APPLY_FAILED - proposals stay proposed; a later catch-up retries"
      bash factory/brain-seed.sh --catch-up || log "SEED_FAILED - a later catch-up retries"
    fi ;;
