...
from the outside it looks like one prompt.
    ...
    python3 factory/cost.py record <run-id>          # fold node usage into the run record
    python3 factory/cost.py report                   # per-run and per-node totals
...
def cmd_record(argv: list[str]) -> int:
    ...
    # The report keys on this and printed `?` for every lap before it was stored
    # (Phase D lap 2, finding D).
    record["run"] = run
    record["nodes"] = nodes
    record["tokens"] = {
...
def cmd_report(_argv: list[str]) -> int:
    ...
    for path in runs:
        ...
        total_out += t["output"]
        usd = r.get("cost_usd")
        # Filename fallback reads the pre-finding-D records, which never stored "run".
        name = r.get("run") or os.path.splitext(os.path.basename(path))[0]
        print(f"{name:<20} {len(r.get('nodes',[])):>5} "
