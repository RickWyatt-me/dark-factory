...
drift, and the entire value of the table is that a node cannot invent a transition.
    ...
    python3 factory/state.py list issues --state accepted
    python3 factory/state.py get gh:issue:3          # or issues/0007.md
    python3 factory/state.py set gh:issue:3 state=accepted priority=high
    python3 factory/state.py body gh:issue:3         # the issue, as a file a node can read
    python3 factory/state.py bump-attempt gh:pr:4    # the fix cap, FACTORY_RULES.md 8
    python3 factory/state.py next                    # what the dispatcher should run
